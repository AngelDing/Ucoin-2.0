...
namespace Ucoin.Framework.ObjectMapper
{
    internal static class CompilationContextExtensions
    {
        ...
        public static void EmitDefault(this CompilationContext context, Type targetType)
        {
            ...
            {
                LocalBuilder local = context.DeclareLocal(targetType);
                context.Emit(OpCodes.Ldloca, local);
            ...
            {
                context.Emit(OpCodes.Ldc_I4_0);
                context.EmitCall(targetType.GetMethod("op_Implicit", BindingFlags.Static | BindingFlags.Public, null,
                    new[] {typeof (int)}, null));
                context.CurrentType = originalType;
                return;
            }
            FieldInfo field = targetType.GetField("Empty", BindingFlags.Public | BindingFlags.Static) ??
            ...
            PropertyInfo property = targetType.GetProperty("Empty", BindingFlags.Public | BindingFlags.Static) ??
            ...
            LocalBuilder targetLocal = context.DeclareLocal(targetType);
